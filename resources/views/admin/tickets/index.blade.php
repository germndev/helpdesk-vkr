    ...
    <div class="content-card p-3 p-md-4">
        ...
        <form method="get" class="admin-ticket-filters mb-2" data-ticket-filter-form>
            <div class="multi-filter" data-multi-filter>
                <button type="button" class="form-select text-start" data-multi-filter-toggle>Ответственный</button>
                <div class="multi-filter-menu" data-multi-filter-menu>
                    @foreach($supportUsers as $support)
                        <label class="multi-filter-option">
                            <input type="checkbox" name="assigned_to[]" value="{{ $support->id }}" @checked(collect($filters['assigned_to'])->contains($support->id) || collect($filters['assigned_to'])->contains((string)$support->id))>
                            <span>{{ $support->display_name }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
            ...
            <div class="multi-filter" data-multi-filter>
                ...
                <div class="multi-filter-menu" data-multi-filter-menu>
                    ...
                    @endforeach
                    <label class="multi-filter-option">
                        <input type="checkbox" name="priority[]" value="__missing" @checked(collect($filters['priority'])->contains('__missing') || ($filters['priority_missing'] ?? false))>
        ...
        <div data-ticket-results>
            @include('admin.tickets._results', ['tickets' => $tickets])
        </div>
    </div>
</div>
@endsection
