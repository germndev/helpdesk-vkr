...
<div class="p-3 p-md-4">
    <div class="content-card content-card-scroll">
        ...
        <section class="ticket-detail-main">
            ...
            <form id="support-ticket-complete-form" action="{{ route('support.tickets.complete', $ticket) }}" method="post">
                @csrf
            </form>

            <form method="post" action="{{ route('support.tickets.update', $ticket) }}" data-inline-edit-form data-inline-start-edit="{{ request()->boolean('edit') ? '1' : '0' }}">
                @csrf
                @method('put')
                ...
                <div class="d-flex justify-content-between align-items-start mb-3 gap-2">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        @if($ticket->status !== 'closed')
                            <button type="submit" class="btn btn-success" form="support-ticket-complete-form">
                                Закрыть заявку
                            </button>
                        @endif
                ...
                <div class="ticket-field">
                    <div class="ticket-field-label">Пользователь</div>
                    <div class="ticket-field-value">{{ $ticket->user->display_name }}</div>
                </div>
        ...
        @include('tickets._chat', [
            ...
            'chatRoute' => route('support.tickets.messages.store', $ticket),
            'chatFetchRoute' => route('support.tickets.messages.index', $ticket),
